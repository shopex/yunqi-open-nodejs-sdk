...
var yunqi = function(server, key, secret){
    this.baseurl = server;
    ...
    this.rpc = function(api, options){
        ...
        if(is_https){
            ...
            params["sign"] = sign(options.type, this.server.path, {}, GetParams, PostParams, this.secret())
        }
        ...
        this.request_url = this.baseurl+"?"+params_str;
        req.end();
    }
}
    ...
    var fp = function(res){
        if(res.statusCode==200){
            res.on('data', function(data){
                ...
                on_success({"code": res.statusCode, request_id: rsp.request_id, "response": rsp});
            });
        }else{
            res.on('data', function(data){
                var rsp = response(res.headers, data);
...
module.exports = function(url, key, secret) {
    return new yunqi(url, key, secret)
}
